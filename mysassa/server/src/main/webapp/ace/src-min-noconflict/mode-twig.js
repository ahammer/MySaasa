ace.define("ace/mode/twig", ["require", "exports", "module", "ace/lib/oop", "ace/mode/text", "ace/mode/javascript", "ace/mode/css", "ace/tokenizer", "ace/mode/twig_highlight_rules", "ace/mode/behaviour/html", "ace/mode/folding/html", "ace/mode/matching_brace_outdent"], function (e, t, n) {
    var r = e("../lib/oop"), i = e("./text").Mode, s = e("./javascript").Mode, o = e("./css").Mode, u = e("../tokenizer").Tokenizer, a = e("./twig_highlight_rules").TwigHighlightRules, f = e("./behaviour/html").HtmlBehaviour, l = e("./folding/html").FoldMode, c = e("./matching_brace_outdent").MatchingBraceOutdent, h = function () {
        this.HighlightRules = a, this.$outdent = new c, this.$behaviour = new f, this.createModeDelegates({"js-": s, "css-": o}), this.foldingRules = new l
    };
    ...
    var r = e("../lib/oop"), i = e("../lib/lang"), s = e("./html_highlight_rules").HtmlHighlightRules, o = e("./text_highlight_rules").TextHighlightRules, u = function () {
        s.call(this);
        ...
        var t = "abs|batch|capitalize|convert_encoding|date|date_modify|default|e|escape|first|format|join|json_encode|keys|last|length|lower|merge|nl2br|number_format|raw|replace|reverse|slice|sort|split|striptags|title|trim|upper|url_encode", n = "attribute|constant|cycle|date|dump|parent|random|range|template_from_string", r = "constant|divisibleby|sameas|defined|empty|even|iterable|odd", i = "null|none|true|false", o = "b-and|b-xor|b-or|in|is|and|or|not", u = this.createKeywordMapper({"keyword.control.twig": e, "support.function.twig": [t, n, r].join("|"), "keyword.operator.twig": o, "constant.language.twig": i}, "identifier");
        for (var a in this.$rules)this.$rules[a].unshift({token: "variable.other.readwrite.local.twig", regex: "\\{\\{-?", push: "twig-start"}, {token: "meta.tag.twig", regex: "\\{%-?", push: "twig-start"}, {token: "comment.block.twig", regex: "\\{#-?", push: "twig-comment"});
        this.$rules["twig-comment"] = [
            {token: "comment.block.twig", regex: ".*-?#\\}", next: "pop"}
        ], this.$rules["twig-start"] = [
            {token: "variable.other.readwrite.local.twig", regex: "-?\\}\\}", next: "pop"},
