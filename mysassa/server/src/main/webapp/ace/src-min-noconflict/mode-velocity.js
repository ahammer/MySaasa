ace.define("ace/mode/velocity", ["require", "exports", "module", "ace/lib/oop", "ace/mode/text", "ace/tokenizer", "ace/mode/velocity_highlight_rules", "ace/mode/folding/velocity", "ace/mode/behaviour/html"], function (e, t, n) {
    var r = e("../lib/oop"), i = e("./text").Mode, s = e("../tokenizer").Tokenizer, o = e("./velocity_highlight_rules").VelocityHighlightRules, u = e("./folding/velocity").FoldMode, a = e("./behaviour/html").HtmlBehaviour, f = function () {
        this.HighlightRules = o, this.foldingRules = new u, this.$behaviour = new a
    };
    ...
    var r = e("../lib/oop"), i = e("../lib/lang"), s = e("./text_highlight_rules").TextHighlightRules, o = e("./html_highlight_rules").HtmlHighlightRules, u = function () {
        o.call(this);
        ...
        this.$rules.start.push({token: "comment", regex: "##.*$"}, {token: "comment.block", regex: "#\\*", next: "vm_comment"}, {token: "string.regexp", regex: "[/](?:(?:\\[(?:\\\\]|[^\\]])+\\])|(?:\\\\/|[^\\]/]))*[/]\\w*\\s*(?=[).,;]|$)"}, {token: "string", regex: '["](?:(?:\\\\.)|(?:[^"\\\\]))*?["]'}, {token: "string", regex: "['](?:(?:\\\\.)|(?:[^'\\\\]))*?[']"}, {token: "constant.numeric", regex: "0[xX][0-9a-fA-F]+\\b"}, {token: "constant.numeric", regex: "[+-]?\\d+(?:(?:\\.\\d*)?(?:[eE][+-]?\\d+)?)?\\b"}, {token: "constant.language.boolean", regex: "(?:true|false)\\b"}, {token: function (i) {
        ...
        }, regex: "[a-zA-Z$#][a-zA-Z0-9_]*\\b"}, {token: "keyword.operator", regex: "!|&|\\*|\\-|\\+|=|!=|<=|>=|<|>|&&|\\|\\|"}, {token: "lparen", regex: "[[({]"}, {token: "rparen", regex: "[\\])}]"}, {token: "text", regex: "\\s+"}), this.$rules.vm_comment = [
            {token: "comment", regex: "\\*#|-->", next: "start"},
            {defaultToken: "comment"}
        ], this.$rules.vm_start = [
